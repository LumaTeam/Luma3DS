...
**Essential-file backups** can be regenerated by holding <kbd>R+A</kbd> at boot. This reruns only the backup portion of the upgrade process and is useful when `/luma/backups` is missing without waiting for a new Luma3DS release.
